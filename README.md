...
- [x] **المرحلة 2** — تجربة الطالب: إدارة السيارات + طلب الغسيل + تتبع الحالة + سجل الطلبات + رفع الصور (Cloudinary)
- [ ] المرحلة 3 — لوحة العامل وإدارة الطلبات
- [ ] المرحلة 4 — الدفع والتقييمات والإشعارات

## رفع الصور (Cloudinary)

رفع صور السيارات يتم عبر Cloudinary. عبّئ المتغيرات في `.env`:
`CLOUDINARY_CLOUD_NAME` و`CLOUDINARY_API_KEY` و`CLOUDINARY_API_SECRET` و`NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME`.

> بدون هذه المتغيرات يرجع endpoint الرفع 503 ويُعطَّل رفع الصور — لكن طلب الغسيل يظل يعمل لأن صورة السيارة **اختيارية**.
